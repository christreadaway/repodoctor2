...
If you get `Cannot find path 'C:\Users\<you>\repodoctor22'` → see **First Time on This Machine** below.
...
```powershell
cd ~\repodoctor22; git pull origin main
```
...
If `~\repodoctor22` doesn't exist yet, paste this **entire block** into PowerShell. It installs Python + Git (skipped if already present), clones the repo, and launches:
...
After this, `cd ~\repodoctor22; .\start.ps1` is all you need.
...
- `~\repodoctor22` resolves to `C:\Users\<you>\repodoctor22` in PowerShell.
- The repo name on GitHub is `repodoctor2` (one 2). The **local folder** is `repodoctor22` (two 2s) — that's intentional, don't "fix" it.
